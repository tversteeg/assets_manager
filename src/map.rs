...
use crate::{entry::CacheEntry, SharedString, UntypedHandle};
...
impl AssetMap {
    ...
    pub fn contains_key(&self, hash: u64, id: &str, type_id: TypeId) -> bool {
        self.get(hash, id, type_id).is_some()
    }
    ...
    pub fn remove(&mut self, hash: u64, id: &str, type_id: TypeId) -> bool {
        self.take(hash, id, type_id).is_some()
    }
    ...
    pub fn iter_for_debug(&self) -> impl Iterator<Item = (&SharedString, &CacheEntry)> + '_ {
        self.map.iter().map(|e| (e.id(), e))
    }
}
